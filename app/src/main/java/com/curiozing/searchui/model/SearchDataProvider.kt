...
object SearchDataProvider {
    ...
    init {
        for(i in 1..100){
            val randomUser = Random.nextInt(usersList.size)
            val userModel = UserModel(name = usersList[randomUser], email = "${usersList[randomUser]}@gmail.com")

            val randomProduct = Random.nextInt(productsList.size)
            val randomBrand = Random.nextInt(brandsList.size)
            val randomPrice = Random.nextInt(300,1500)

            val productModel = ProductModel(name = productsList[randomProduct], price =randomPrice.toDouble(), brand = brandsList[randomBrand])
